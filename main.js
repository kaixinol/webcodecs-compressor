    ...
    return {
        ...
        async _detectCodecs() {
            try {
                ...
                const results = await Promise.all(tests.map(async (t) => {
                    try {
                        const ok = await mb.canEncodeVideo(
                            t.mbCodec, { width: 1280, height: 720, bitrate: 1e6 }
                        );
                        return {
                            id: t.id, label: t.label, supported: ok,
                            tooltip: ok ? '' : `Browser does not support encoding ${t.label}`,
                        };
                    } catch {
                        return {
                            id: t.id, label: t.label, supported: false,
                ...
                this.codecs = [{ id: 'h264', label: 'H.264 (MP4)', supported: true, tooltip: '' }];
            }
        },
        ...
        async setFile(file) {
            ...
            try {
                ...
                if (videoTrack) {
                    ...
                    const par = (ar && ar.numerator != null && ar.denominator != null && ar.denominator !== 0)
                        ? `${ar.numerator}:${ar.denominator}`
                        : '1:1';
                    const cs = videoTrack.colorSpace;
                    const colorName = (cs && cs.name) ? cs.name : (cs ? JSON.stringify(cs) : 'unknown');
                    videoInfo = {
                ...
                const totalBitrate = effDuration > 0
                    ? (size * 8) / effDuration
                    : null;
                ...
                this.metadata = {
                    ...
                    totalBitrate,
                    totalBitrateStr: totalBitrate ? `${(totalBitrate / 1000).toFixed(0)} kbps` : 'N/A',
                    video: videoInfo,
                    audio: audioInfo,
                };
        ...
        setResolution(id) {
            ...
            if (preset.height != null && preset.height > srcH) {
                ...
                setTimeout(() => { this.warning = null; }, 3000);
                return;
            }
        ...
        async startProcessing() {
            ...
            if (this.settings.resolution === 'custom') {
                ...
                if (this.warning) { this.error = this.warning; return; }
                if (!this.settings.customWidth || !this.settings.customHeight) {
                    this.error = 'Please enter both width and height for custom resolution.';
                    return;
                }
            }
            ...
            try {
                ...
                const result = await processVideo({
                    ...
                    onStatus: (msg) => { this.statusMessage = msg; },
                    onConversionReady: (conv) => { this.currentConversion = conv; },
                });
                ...
                this.statusMessage =
                    `Done! ${this.formatSize(result.outputSize)} (${pct}% of source)`;
        ...
        /* ── helpers ─────────────────────────────────────────────────── */
        _channelLabel(n) {
            const map = { 1: 'Mono', 2: 'Stereo', 6: '5.1 Surround', 8: '7.1 Surround' };
            return map[n] || `${n}ch`;
        },
        ...
        _triggerDownload(url, filename) {
            ...
            a.click();
            requestAnimationFrame(() => { if (a.parentNode) a.remove(); });
        },
    };
}
